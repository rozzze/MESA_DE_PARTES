    ...
    <div class="bg-gradient-to-r from-[#002D64] via-[#0F3D59] to-[#002D64] p-6 rounded-xl shadow-lg mb-6 relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <flux:text class="text-white text-sm lg:text-base font-semibold opacity-90">
                {{ __('Maneja todos tus Roles en el sistema') }}
            </flux:text>
        </div>
    </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-[#0D0D0D] p-6 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 relative overflow-hidden mt-6">
            ...
            <div class="flex-1 relative z-10">
                <div class="flex items-center gap-3 mb-2">
                    <div class="p-2 bg-gradient-to-r from-[#0477BF] to-[#002D64] rounded-lg shadow-md">
                        {{-- Icono representativo de roles --}}
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h-5m-2 0h-2M15 4a3 3 0 00-3 3v1h4V7a3 3 0 00-3-3z"></path>
                        </svg>
                    </div>
                    <h3 class="text-lg font-semibold text-[#0D0D0D] dark:text-white">Gestión de Roles</h3>
                </div>
                <p class="text-sm text-gray-600 dark:text-gray-400 ml-12">Administra los roles y sus permisos en el sistema</p>
            </div>
            ...
            <a href="{{ route("roles.create") }}"
               class="group relative inline-flex items-center justify-center px-8 py-3 overflow-hidden text-sm font-semibold text-white rounded-xl bg-gradient-to-r from-[#0477BF] to-[#002D64] hover:from-[#002D64] hover:to-[#0477BF] focus:ring-4 focus:outline-none focus:ring-[#0477BF]/50 dark:focus:ring-[#002D64]/50 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl relative z-10">
                ...
                <span class="relative">Crear Rol</span>
            </a>
        </div>
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-700 dark:text-gray-300">
                    <thead class="text-xs uppercase bg-gray-50 dark:bg-[#0F3D59] text-[#002D64] dark:text-white border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 font-semibold">Permisos</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($roles as $role)
                            <tr class="hover:bg-gradient-to-r hover:from-[#F2CB05]/5 hover:to-[#B88900]/5 dark:hover:bg-[#0F3D59]/20 transition-all duration-300 transform hover:translate-x-1">
                                <td class="px-6 py-4 font-medium text-[#0D0D0D] dark:text-white">{{ $role->id }}</td>
                                <td class="px-6 py-4 text-gray-800 dark:text-gray-200">{{ $role->name }}</td>
                                <td class="px-6 py-4">
                                    @if ($role->permissions)
                                        <div class="flex flex-wrap gap-2"> 
                                            @foreach ($role->permissions as $permission)
                                                {{-- Badge de permisos con degradado amarillo/dorado --}}
                                                <flux:badge class="px-3 py-1 text-xs font-semibold rounded-full bg-gradient-to-r from-[#F2C84B] to-[#F2CB05] text-[#0D0D0D] shadow-sm transform transition-all duration-300 hover:scale-105">{{ $permission->name }}</flux:badge>
                                            @endforeach
                                        </div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-2 justify-center">
                                        <a href="{{ route('roles.show', $role->id) }}"
                                           class="text-white bg-gradient-to-r from-[#002D64] to-[#0F3D59] hover:from-[#0F3D59] hover:to-[#002D64] focus:ring-4 focus:outline-none focus:ring-[#0F3D59]/50 dark:focus:ring-[#002D64]/50 font-medium rounded-lg text-sm px-4 py-2 text-center transition-all duration-200 hover:scale-105 shadow-lg hover:shadow-xl">
                                            Mostrar
                                        </a>
