    ...
    <div class="bg-gradient-to-r from-[#002D64] via-[#0F3D59] to-[#002D64] p-6 rounded-xl shadow-lg mb-6 relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <flux:subheading class="text-white text-sm lg:text-base font-semibold opacity-90 mt-2">
                {{ __('Aquí podrá ver la información completa del usuario.') }}
            </flux:subheading>
        </div>
    </div>
        ...
        <div class="relative z-10"> {{-- Contenido de los datos --}}
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-semibold text-[#0D0D0D] dark:text-white">{{ __('Información del Usuario') }}</h2>
                </div>
            ...
            <div class="mt-6 space-y-6">
                ...
                <flux:input disabled label="{{ __('Nombre de Usuario') }}" value="{{ $user->name }}" />
                <flux:input disabled label="{{ __('Correo Electrónico') }}" value="{{ $user->email }}" />

                {{-- Mostrar roles si es necesario --}}
                @if($user->roles->isNotEmpty())
                    <div class="flex flex-wrap items-center gap-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Roles Asignados:') }}</label>
                        @foreach ($user->getRoleNames() as $role)
                            <span class="px-3 py-1 text-xs font-semibold rounded-full bg-[#F2CB05]/20 text-[#B88900] dark:bg-[#B88900]/20 dark:text-[#F2CB05]">{{ $role }}</span>
                        @endforeach
                    </div>
                @else
                ...
                {{-- Puedes añadir más campos aquí si $user tiene más propiedades que quieras mostrar --}}
                {{-- Por ejemplo: --}}
                {{-- <flux:input disabled label="{{ __('Fecha de Creación') }}" value="{{ $user->created_at->format('d/m/Y H:i') }}" /> --}}
            </div>
        </div>
    </div>
</div>